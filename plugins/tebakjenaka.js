...
`.trim()
    conn.tebakjenaka[id] = [
        await conn.sendBut(m.chat, caption, `Recode by Fajar`, 'Bantuan', `.teje`, m),
        json, poin,
        setTimeout(async () => {
            if (conn.tebakjenaka[id]) await conn.sendBut(m.chat, `Waktu habis!\nJawabannya adalah *${json.result.jawaban}*`, `Recode by Fajar`, 'Teka Teki', `.tebakjenaka`, conn.tebakjenaka[id][0])
