...
`.trim()
    conn.tebaktebakan[id] = [
        await conn.sendBut(m.chat, caption, `Recode by Fajar`, 'Bantuan', `.tebek`, m),
        json, poin,
        setTimeout(async () => {
            if (conn.tebaktebakan[id]) await conn.sendBut(m.chat, `Waktu habis!\nJawabannya adalah *${json.jawaban}*`, `Recode by Fajar`, 'Tebak tebakan', `.tebaktebakan`, conn.tebaktebakan[id][0])
